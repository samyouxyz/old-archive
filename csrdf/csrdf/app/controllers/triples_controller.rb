...
class TriplesController < ApplicationController
	...
	def show
		...
		query ="PREFIX csr: <http://csrdf.com/restaurant/>
		...
		$result_query=""
		SPARQL_query_GET("http://localhost:3030/ds/query", query )
		render plain: $result_query
	...
	def do_test
	  ...
	  triple_post = "@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
@prefix owl: <http://www.w3.org/2002/07/owl#> . 
@prefix foaf: <http://xmlns.com/foaf/0.1/>. 
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> . 
@prefix db: <http://dbpedia.org/> . 
...
<http://csrdf.com/restaurant/#{name.downcase.strip.gsub(' ', '_').gsub(/[^\w-]/, '')}> 
  foaf:name \"#{name}\"; 
  dbo:cuisine \"#{cuisine}\"; 
  dbp:phonenumber \"#{phone}\"; 
  dbo:location \"#{location}\"; 
  dbp:openinghours \"#{openinghours}\"; 
  dbp:price \"#{price}\"; 
  rdfs:comment \"#{comment}\"; 
  geo:long \"\"; 
  geo:lat \"\";
  dbo:industry dbr:Restaurant .
"

	render plain: triple_post
	POST("http://localhost:3030/ds","default",triple_post)
...
# ------------------------------------------------
	...
	def send_body(dataset, graph, file, method)
	  ...
	  # end
	  st=file.size.to_s
	  headers = {"User-Agent"=>"SOH/Fuseki 1.0.0", "Content-Type"=>"text/turtle;charset=utf-8;charset=utf-8", "Content-Length"=>st}
	  # headers.merge!($headers)
	  # headers[$hContentType] = "text/turtle;charset=utf-8;charset=utf-8"
	  # headers[$hContentLength] = file.size+1
	  ## p headers
	  ...
	  begin
	    response = http.request(request) { |res| 
	      ...
	      res.read_body do |segment|
	        $result_query = $result_query +segment
	      end
	    }
	    case response
	    when Net::HTTPSuccess, Net::HTTPRedirection
	...
	def SPARQL_query_GET(service, query)
	  args = { "query" => query }
